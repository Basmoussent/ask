    ...
    def send(self):
        ...
        try:
            ...
            if self.method == "GET":
                ...
                r = requests.post(self.url, data=self.params, timeout=10)
            elif self.method == "PUT":
                r = requests.put(self.url, data=self.params, timeout=10)
            elif self.method == "DELETE":
                r = requests.delete(self.url, data=self.params, timeout=10)
            elif self.method == "PATCH":
                r = requests.patch(self.url, data=self.params, timeout=10)
            elif self.method == "HEAD":
                r = requests.head(self.url, params=self.params, timeout=10)
            else:
